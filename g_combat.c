...
//			VectorMA( vec, 0.1, tr.plane.normal, tr.endpos );	// lift it above the ground slightly
		...
		// JOSEPH 18-DEC-98
		SurfaceSpriteEffect(SFX_SPRITE_SURF_BLOOD_POOL, 8, 8, tr.ent, tr.endpos, tr.plane.normal);
		/*gi.WriteByte (svc_temp_entity);
...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	if (dm_realmode->value && attacker != targ && attacker->client)
	{
		dmg *= 4;
	}
...
//		dmg *= 2;
	...
	// no knockback if movement is disabled by a trigger_push
	if (targ->client && targ->client->nomove)
...
// figure momentum add
	...
	// END JOSEPH
	{
		if ((knockback) && (targ->movetype != MOVETYPE_NONE) && (targ->movetype != MOVETYPE_BOUNCE) && (targ->movetype != MOVETYPE_PUSH) && (targ->movetype != MOVETYPE_STOP))
		{
			...
			{
				VectorScale (dir, 500.0 * (float)knockback / mass, kvel);
				// prevent hurting teammates by shotgunning them into the ground
				if (!dmg && targ->velocity[2] + kvel[2] < -600)
					kvel[2] = 0;
			}
			// END JOSEPH
			...
			if ((targ->svflags & SVF_MONSTER) || (client))
				if (hasarmor)
				{
...

					{
						...
						if (rval > 66)
							gi.sound(targ, CHAN_AUTO, gi.soundindex("weapons/bullethit_armor1.wav"), 1, ATTN_NORM, 0);
						else if (rval > 33)
							gi.sound(targ, CHAN_AUTO, gi.soundindex("weapons/bullethit_armor2.wav"), 1, ATTN_NORM, 0);
						else
							gi.sound(targ, CHAN_AUTO, gi.soundindex("weapons/bullethit_armor3.wav"), 1, ATTN_NORM, 0);
					}
