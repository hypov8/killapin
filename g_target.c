...
void target_laser_start (edict_t *self)
{
	...
	// set the color
	if (self->spawnflags & 2)
		self->s.skinnum = 0xf2f2f0f0; //lt blue
	else if (self->spawnflags & 4)
		self->s.skinnum = 0xd0d1d2d3; //orange
	else if (self->spawnflags & 8)
		self->s.skinnum = 0xf3f3f1f1; //blue
	else if (self->spawnflags & 16)
		self->s.skinnum = 0xdcdddedf; //red
	else if (self->spawnflags & 32)
		self->s.skinnum = 0xe0e1e2e3; //drk blue
...
void target_timer_use (edict_t *self, edict_t *other, edict_t *activator)
{
    ...
    if (TeamID>=0 && TeamID<10 && activator && activator->client)
    {
        ...
        // stop
        {
            ...
            {
                gi.cprintf(activator, PRINT_HIGH, "you haven't started timer %i yet!\n", TeamID);
            }
        }
        else
        // start
