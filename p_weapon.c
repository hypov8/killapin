...
//CDEATH - Removed static declaration so that external files can link
...
qboolean Pickup_Weapon (edict_t *ent, edict_t *other)
{
	...
	if (!(other->client->pers.inventory[index]))
	{
		...
		// if they pick a new weapon up make them mortal
	...
	*/
	...
	if (deathmatch_value)
	{
		if (!other->client->newweapon && other->client->pers.weapon != ent->item && auto_reload && (other->client->pers.weapon == FindItem ("Pistol") || other->client->pers.weapon == FindItem ("Pipe") ))
			other->client->newweapon = ent->item;
	}
	else
	{
...
/*	else if (!strcmp(ent->client->pers.weapon->pickup_name, "FlameThrower"))
	...
	// END JOSEPH
//CDEATH
	else if (!strcmp(ent->client->pers.weapon->pickup_name, "Harpoon"))
		ent->client->clip_index = CLIP_HARPOON;
//END CDEATH
}
...
/*	else if (!strcmp(item->pickup_name, "FlameThrower"))
	...
	// END JOSEPH
//CDEATH
	else if (!strcmp(item->pickup_name, "Harpoon"))
		return CLIP_HARPOON;
//END CDEATH
/*	else if (!strcmp(item->pickup_name, "AmmoDrop"))
		return CLIP_AMMOBOX;
	else if (!strcmp(item->pickup_name, "HealthDrop"))
		return CLIP_HEALTH;*/
...
/*		else if (!strcmp(ent->client->pers.weapon->pickup_name, "AmmoDrop") || !strcmp(ent->client->pers.weapon->pickup_name, "HealthDrop"))
		{
			ent->client->ps.num_parts++;
			ent->client->ps.model_parts[PART_LEGS].modelindex = gi.modelindex("models/weapons/v_ammo/hand.mdx");
		}*/
	}
...
void Think_FlashLight (edict_t *ent)
{
	...
	if (ent->health < 1 || ent->solid == SOLID_NOT)
		return;
...
void Drop_Weapon (edict_t *ent, gitem_t *item)
{
	...
	if ((int)(dmflags->value) & DF_WEAPONS_STAY)
	{
		gi.cprintf (ent, PRINT_HIGH, "Weapons are not dropable\n");
		return;
	}
...
*/
	...
	{
		// cancel/reverse activation if another weapon is wanted
		if (deathmatch_value && ent->client->newweapon)
		{
	...
	{
		if (ent->client->weaponstate == WEAPON_FIRING && !strcmp(ent->client->pers.weapon->pickup_name, "Grenade") && ent->client->ps.gunframe >= 18)
			goto noskipfiring;
...
/*			if (pause_frames) // messes up weapon bobbing
			{
...
void Weapon_Blackjack (edict_t *ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->weaponthink != Weapon_Blackjack)
		return;
...
void Weapon_Crowbar (edict_t *ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->weaponthink != Weapon_Crowbar)
		return;
...
void Weapon_Pistol (edict_t *ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->weaponthink != Weapon_Pistol)
		return;
...
// gi.dprintf ("silencer shots left: %d\n", ent->client->pers.silencer_shots);
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->weaponthink != Weapon_SPistol)
		return;
...
void Weapon_GrenadeLauncher (edict_t *ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.weapon->weaponthink != Weapon_GrenadeLauncher)
		return;
...
void Weapon_Grenade_Fire (edict_t *ent)
{
	...
	if (ent->client->ps.gunframe == 22)
	{
		// trigger male throwing(ish) animation
		if (!IsFemale(ent))
			ent->client->attacked = true;
		ent->client->ps.gunframe++;
		return;
	}
	if (ent->client->ps.gunframe == 24)
	{
		// trigger female throwing(ish) animation
		if (IsFemale(ent))
			ent->client->attacked = true;
		ent->client->ps.gunframe++;
		return;
	...
	Weapon_Grenade_Release (ent, 800);
...
void Weapon_Grenade (edict_t *ent)
{
	...
	static int	fire_frames[]	= {18, 22, 24, 26, 29, 0};

	Weapon_Generic (ent, 6, 29, 33, 37, pause_frames, fire_frames, Weapon_Grenade_Fire);
}

void Weapon_Drop_Fire (edict_t *ent)
{
	if (ent->client->ps.gunframe == 5)
	{
		gitem_t *it = FindItem (ent->client->pers.weapon->tag ? "Large Health" : "Ammo Box");
		edict_t *drop = Drop_Item(ent, it);
		drop->flags |= FL_IMPACTSOUND;
		drop->s.angles[YAW] = ent->s.angles[YAW];
		if (ent->client->pers.weapon->tag)
			drop->s.angles[YAW] -= 90;
		drop->mins[1] = drop->mins[0] = -9;
		drop->maxs[1] = drop->maxs[0] = 9;
		ent->client->drop_frame = level.framenum + 10;
	}
	else if (level.framenum < ent->client->drop_frame)
		return;
	ent->client->ps.gunframe++;
}

void Weapon_Drop (edict_t *ent)
{
	static int	pause_frames[]	= {0};
	static int	fire_frames[]	= {5, 8, 0};
	...
	if (ent->client->weaponstate == WEAPON_ACTIVATING)
	{
		...
		{
			if (level.framenum >= ent->client->drop_frame)
			{
				ent->client->weaponstate = WEAPON_READY;
	...
	Weapon_Generic (ent, 4, 11, 26, 31, pause_frames, fire_frames, Weapon_Drop_Fire);

	if (ent->client->ps.gunframe > 4 && ent->client->ps.gunframe < 9)
		ent->client->ps.model_parts[PART_HEAD].invisible_objects = 1;
	else
		ent->client->ps.model_parts[PART_HEAD].invisible_objects = 0;
}
