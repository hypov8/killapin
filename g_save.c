...
//CDEATH
//	capture_limit = gi.cvar("capturelimit", "0", CVAR_SERVERINFO);
	harpoon_enable = gi.cvar("harpoon_enable", "1", 0);
	harpoon_max_distance = gi.cvar("harpoon_maxdist", "0", 0);
	harpoon_cable = gi.cvar("harpoon_cable", "1", 0);
	harpoon_speed_fly = gi.cvar("harpoon_speed_fly", HARPOON_SPEED_FLY_DEFAULT, 0);
	harpoon_speed_pull = gi.cvar("harpoon_speed_pull", HARPOON_SPEED_PULL_DEFAULT, 0);
	harpoon_damage = gi.cvar("harpoon_damage", HARPOON_DAMAGE_DEFAULT, 0);
//	gi.cvar("_URL", Address_URL, CVAR_SERVERINFO | CVAR_NOSET);
	hud_flash = gi.cvar("hud_flash", "1", 0);	//Disable or set flash speed for HUD flash when capturing a control point
//END CDEATH
...
// Ridah, new cvar's
	...
	dm_realmode = gi.cvar( "dm_realmode", "0", CVAR_LATCH|CVAR_SERVERINFO|CVAR_NORELOAD);
	
	g_mapcycle_file = gi.cvar( "g_mapcycle_file", "", 0);
// Ridah, done.

	teams = gi.cvar("teams", "3", CVAR_LATCH | CVAR_SERVERINFO);
	if (teams->value < 2)
		gi.cvar_set("teams", "2");
	else if (teams->value > 3)
		gi.cvar_set("teams", "3");
