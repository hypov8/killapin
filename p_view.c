...
//===================================
	...
	if (ent->deadflag && !ent->client->chase_target)
	{
		VectorClear (angles);

		if (ent->client->killer)
		{
			if (ent->client->killer->solid != SOLID_NOT)
			{
				vec3_t	spot1;
				trace_t	trace;
				VectorCopy (ent->s.origin, spot1);
				spot1[2] += 40;
				trace = gi.trace (spot1, vec3_origin, vec3_origin, ent->client->killer->s.origin, ent, MASK_OPAQUE);
				if (trace.fraction == 1.0)
				{
					// track the killer
					LookAtKiller(ent, NULL, ent->client->killer);
					ent->client->ps.viewangles[YAW] = ent->client->killer_yaw;
				}
			}
			if (ent->client->dead_frames >= 4)
			{
				if (ent->client->dead_frames < 10)
					ent->client->ps.viewangles[PITCH] -= 10;
				else if (ent->client->dead_frames > 30 && ent->client->ps.viewangles[PITCH] < 60)
					ent->client->ps.viewangles[PITCH] += 10;
			}
		}
		ent->client->dead_frames++;
		if (ent->client->dead_frames > 1)
		{
			ent->client->ps.pmove.pm_flags |= PMF_CHASECAM;
			ent->client->ps.fov = 90;
			VectorSet(ent->client->ps.viewoffset, 0, 0, 36);
			return;
		}
	}
	else
...
void SV_CalcGunOffset (edict_t *ent)
{
	...
	if (!ent->client->pers.weapon || ent->client->pers.hand == 2)
		return;
	...
	if (ent->client->ping < 200)
	{
		// gun angles from delta movement (modified to keep gun aligned)
		for (i=0 ; i<3 ; i++)
		{
			delta = ent->client->oldviewangles[i] - ent->client->ps.viewangles[i];
			if (i == PITCH)
				delta += (ent->velocity[2] + sv_gravity->value * 0.012) * 0.1; // move gun up/down when jumping
			if (delta > 180)
				delta -= 360;
			if (delta < -180)
				delta += 360;
			...
			if (delta < -45)
				delta = -45;
			if (i == YAW)
				ent->client->ps.gunangles[ROLL] += (ent->client->pers.hand == 1 ? 0.15 : -0.15) * delta;
			else
				ent->client->ps.gunangles[i] += 0.2 * delta;
		}
	}

	// disable bobbing when firing the flamethrower because the flame doesn't move with it
...
void SV_CalcBlend (edict_t *ent)
{
	...
	if (contents & CONTENTS_SOLID)
		SV_AddBlend(0.0, 0.0, 0.0, 0.6, ent->client->ps.blend);
	...
	else if (contents & CONTENTS_WATER)
	{
		...
		start[2] += 512;
		tr = gi.trace (start, NULL, NULL, ent->s.origin, NULL, MASK_WATER);
		if (tr.fraction < 1 && (tr.surface->flags & SURF_TRANS33))
			a *= (2 - tr.fraction) * 0.5;
		SV_AddBlend (0.03, 0.04, 0.03, a, ent->client->ps.blend);
	}
	...
	if (ent->deadflag && (ent->client->ps.pmove.pm_flags & PMF_CHASECAM) && !(ent->flags & FL_CHASECAM))
	{
...
/*	if ((ent->client->oldvelocity[2] < 0) && (ent->velocity[2] > ent->client->oldvelocity[2]) && (!ent->groundentity)) // causes footsteps while using harpoon
	{
	...
	// END JOSEPH
	{
		...
		if (!deathmatch_value || (!((int)dmflags->value & DF_NO_FALLING) && !ent->client->resp.is_boss) )
...
void P_WorldEffects (void)
{
	...
	{
		// JOSEPH 13-MAY-99 
		gi.sound (current_player, CHAN_AUTO, gi.soundindex("actors/player/male/watr_out.wav"), 1, ATTN_NORM, 0); // CHAN_BODY is overridden by footsteps
		// END JOSEPH
		current_player->flags &= ~FL_INWATER;
	}
...
void G_SetClientEffects (edict_t *ent)
{
	...
	if (ent->client->invincible_framenum > level.framenum)
	{
		...
		else if (remaining > 30 || (remaining & 4) )
		{
...
void G_SetClientSound (edict_t *ent)
{
	...
	if (ent->waterlevel && (ent->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
		ent->s.sound = snd_fry;
	else if (ent->health > 0 && ent->client->resp.is_boss && ent->client->invincible_framenum > level.framenum && ent->client->invincible_framenum == level.invincible_boss)
		ent->s.sound = gi.soundindex("killapin/invinc.wav");
	...
	else if (strcmp (weap, "weapon_flamethrower") == 0 && ent->waterlevel < 3)
...
/*	if (!strcmp(client->pers.weapon->classname, "weapon_ammodrop") || !strcmp(client->pers.weapon->classname, "weapon_healthdrop"))
	{
		weapontype = WEAPON_GENERIC;
	}
	else*/ if (!strcmp(client->pers.weapon->classname, "weapon_harpoon"))
	{
		weapontype = WEAPON_GENERIC;
		if (client->buttons & BUTTON_ATTACK)
			ent->client->attacked = true;
	}
	else if (client->attacked)
	{
	...
	if (!client->anim_run && !client->anim_slide && client->anim_priority != ANIM_ATTACK && client->attacked)
		goto newanim;
...
newanim:
	// keep swinging if the attack button is held down
	if (!weapontype && client->last_weapontype == WEAPON_MELEE && (client->buttons & BUTTON_ATTACK))
	{
		weapontype = client->last_weapontype;
		client->attacked = true;
	}
	...
	if (!weapontype && client->last_weapontype)
	{
		// set weapontype for post-attack animation
		weapontype = client->last_weapontype;
		client->last_weapontype = WEAPON_NONE;
	}
	else
...
//		if (ent->s.frame != FRAME_jump_07)
	...
	else if (weapontype && client->attacked)
	{	// standing attack
...
attack:
		...
		else
		{
			...
			case WEAPON_MELEE:
				if (!strcmp(client->pers.weapon->classname, "weapon_grenade"))
				{
					if (IsFemale(ent))
					{
						ent->s.frame = FRAME_melee4_01;
						client->anim_end = FRAME_melee4_04;
					}
					else
					{
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	if (ent->client->chase_target)
	{
		if (ent->client->chasemode == EYECAM_CHASE || ent->client->chase_target->client->chase_target)
		{
			G_SetStats (ent);
			goto updatescore;
		}
...
updatescore:
	...
	{
		// allow more time for cheat checks
		if (ent->client->resp.checkframe[1] && ent->client->resp.checked < 100)
			ent->client->resp.checkframe[1]++;
		return;
	}

	// update extended HUD
	if (ent->client->resp.is_boss && !((level.framenum - ent->client->resp.enterframe) % 10))
		UpdateStatusHUD(ent);
	else if (!ent->client->resp.is_boss && ent->client->resp.exhud[0])
		SetExtendedHUD(ent, "", true);

	if (ent->client->resp.message[0] && level.framenum >= ent->client->resp.message_frame)
	{
		ent->client->resp.message[0] = 0;
	...
	if (level.framenum > ent->client->resp.checkframe[1])
	{
		if (ent->client->resp.checkframe[1] && ent->client->resp.checked < 100)
		{
			// didn't get a valid response to last check. kick?
		}
