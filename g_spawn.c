...
//CDEATH
	{ "power_control_point", SP_Power_Control },
	{ "power_hired_help", SP_Power_Dummy },
	{ "power_cast_trader", SP_Power_Dummy },
//Handle cast_dog. Some maps have dogs but Monkey Mod no longer allows them.
	{ "cast_dog", SP_Power_Dummy },
//END CDEATH
		...
		if (ent != g_edicts)
		{
			...
			if (!strcmp(ent->classname, "info_player_deathmatch"))
			{
				if (!anti_spawncamp->value &&
					((!strcmp(level.mapname, "kpdm5") && VectorCompare(ent->s.origin, spawnvecs[0]))
	...
	if (allow_map_voting && num_maps)
	{
		...
		if (kpded2)
		{
			...
			for (i=1; i<=num_vote_set; i++)
			{
				...
				vote_nopic[i] = !CheckFileExists(buf); // not all maps have a pic so check it exists
				dlindex(vote_nopic[i] ? "pics/mm/nopic.pcx" : buf);
			}
		}
	}
	else
...
"xr	-20 "
"if 24 "
"	picn /pics/mm/team1.tga "
"endif "
"if 25 "
"	picn /pics/mm/team2.tga "
"endif "
"if 29 "
"	picn /pics/mm/team3.tga "
"endif "
...
"xr	-40 "
"yv	100 "
"picn /pics/team1_icon.tga "
"yv	165 "
"picn /pics/team2_icon.tga "
"yv	210 "
"picn /pics/team3_icon.tga "
...
// hack to allow team recognition in client
"steams "
;

char *teamplayDM_statusbar2 =
...
void SP_worldspawn (edict_t *ent)
{
	...
	{
		gi.error("Cannot play this map in DEATHMATCH mode.");
		return;
	}
	...
	// Teamplay
	if (ent->head == 1)
		teams->value = 2; // force 2 teams
	else
		teams->value = atoi(teams->string);
	...
	// status bar program
	{
		char str[1024];
		int length;
		strcpy(str, teams->value == 2 ? teamplayDM_statusbar2 : teamplayDM_statusbar);
		length = strlen(str);
		// pad it to the start of next configstring to allow extension for crew status
		if (length % MAX_QPATH)
		{
			memset(str + length, ' ', MAX_QPATH - (length % MAX_QPATH));
			length += MAX_QPATH - (length % MAX_QPATH);
			str[length] = 0;
		}
		level.cs_statusbarex = CS_STATUSBAR + length / MAX_QPATH;
	...
	gi.soundindex ("*gasp1.wav");		// gasping for air
	gi.soundindex ("*gasp2.wav");		// head breaking surface, not gasping
...
//	gi.modelindex ("#w_grenade launcher.mdx");	// CLIP_GRENADES
	gi.modelindex ("#w_nad.mdx");				// CLIP_GRENADES
//	gi.modelindex ("#w_bazooka.mdx");			// CLIP_ROCKETS
//	gi.modelindex ("#w_flamethrower.mdx");		// CLIP_FLAMEGUN
//CDEATH
	gi.modelindex ("#w_harpoon.mdx");			// CLIP_HARPOON
//END CDEATH
//	gi.modelindex ("#w_ammo.mdx");				// CLIP_AMMOBOX
//	gi.modelindex ("#w_health.mdx");			// CLIP_HEALTH
	// ----------------------------------------------------------------------------------
